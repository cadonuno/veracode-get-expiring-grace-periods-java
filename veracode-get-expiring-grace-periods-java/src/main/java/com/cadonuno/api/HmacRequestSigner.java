...
public class HmacRequestSigner {
     ...
     */
    public static String getVeracodeAuthorizationHeader(final URL url, final String httpMethod)
            throws InvalidKeyException, NoSuchAlgorithmException, IllegalStateException, UnsupportedEncodingException {
        final String urlPath = (url.getQuery() == null)
                ? url.getPath()
                : url.getPath().concat("?").concat(url.getQuery());
        final String data = String.format(DATA_FORMAT, Credentials.VERACODE_API_ID, url.getHost(), urlPath, httpMethod);
        final String timestamp = String.valueOf(System.currentTimeMillis());
        final String nonce = DatatypeConverter.printHexBinary(generateRandomBytes(16)).toLowerCase(Locale.US);
        final String signature = getSignature(Credentials.VERACODE_API_SECRET, data, timestamp, nonce);
        return String.format(HEADER_FORMAT, VERACODE_HMAC_SHA_256, Credentials.VERACODE_API_ID, timestamp, nonce, signature);
    }
